...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding=True):
        ...
        The token embeddings would too, except due to the parameter sharing (weight tying)
        these params are actually used as weights in the final layer, so we include them.
        """
    ...
    @classmethod
    def from_pretrained(cls, model_type, override_args=None, checkpoint=None):  # noqa: D102
        ...
        sd = model.state_dict()
        sd_keys = sd.keys()
        # discard the mask / buffer stored as a bias since it's not a param
        sd_keys = [k for k in sd_keys if not k.endswith('.attn.bias')]

        if checkpoint is not None:
            sd_keys_ckpt = checkpoint.keys()
            sd_keys_ckpt = [
                k for k in sd_keys_ckpt if not k.endswith('.attn.masked_bias')
                ]
            sd_keys_ckpt = [k for k in sd_keys_ckpt if not k.endswith('.attn.bias')]
            assert len(sd_keys_ckpt) == len(sd_keys), \
                f"mismatched keys: {len(sd_keys_ckpt)} != {len(sd_keys)}"

            for k in sd_keys_ckpt:
                assert checkpoint[k].shape == sd[k].shape
                with torch.no_grad():
                    sd[k].copy_(checkpoint[k])

        return model

    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):  # noqa: D102
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        back into the model each time. Most likely you'll want to make sure
        to be in model.eval() mode of operation for this.
        """
